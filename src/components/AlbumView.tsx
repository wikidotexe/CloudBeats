...
import { Play, ArrowLeft, Clock, Disc3 } from "lucide-react";
  ...
  return (
    <div className="p-4 md:p-8 overflow-y-auto scrollbar-thin animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row gap-4 sm:gap-8 mb-6 md:mb-10">
        {coverSrc ? (
          <img src={coverSrc} alt={album.name} className="w-32 h-32 sm:w-56 sm:h-56 rounded-2xl object-cover shadow-2xl mx-auto sm:mx-0 shadow-primary/10" />
        ) : (
          <div className="w-32 h-32 sm:w-56 sm:h-56 rounded-2xl bg-gradient-to-br from-secondary to-secondary/30 flex items-center justify-center text-4xl sm:text-6xl text-muted-foreground/30 mx-auto sm:mx-0">
            <Disc3 className="w-16 h-16 sm:w-24 sm:h-24" />
          </div>
        )}
        <div className="flex flex-col justify-end text-center sm:text-left">
          <p className="text-[10px] sm:text-xs uppercase tracking-[0.2em] text-primary font-bold mb-1 md:2">Album</p>
          <h2 className="font-display text-2xl sm:text-5xl font-bold text-foreground mb-2 md:4 leading-tight">{album.name}</h2>
          <p className="text-xs sm:text-base text-muted-foreground mb-4 md:6">
            {album.artist} {album.year && `• ${album.year}`} • {album.songCount} {album.songCount === 1 ? 'song' : 'songs'}
          </p>
          <button
            onClick={() => currentSongs.length > 0 && playSong(currentSongs[0], currentSongs)}
        ...
        <div className="mt-1 md:mt-2 space-y-0.5 md:space-y-1">
          {currentSongs.map((song, i) => {
            ...
            return (
              <button
                key={song.id}
                onClick={() => playSong(song, currentSongs)}
